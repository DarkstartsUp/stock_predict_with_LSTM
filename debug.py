...
class Data:
    ...
    def get_train_and_valid_data(self):
        feature_data = self.norm_data[:, :, :self.train_num]       # 取出归一化后的数据的训练部分
        label_data = self.label_data[:, :, self.config.time_step: self.train_num]   # 将延后7天的数据作为label

        # 每time_step行数据会作为一个样本，两个样本错开一日，比如：1-30日，2-31日
        train_x = [feature_data[:, :, i:i+self.config.time_step] for i in range(self.train_num-self.config.time_step)]

        train_y = np.transpose(label_data, (2, 0, 1))
